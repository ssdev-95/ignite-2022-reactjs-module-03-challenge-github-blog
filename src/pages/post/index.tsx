...
export function Post() {
  ...
  const [issue, setIssue] = useState<FullPost>({} as FullPost)

  const issueNumber = params.get('n')

  useEffect(() => {
    issueNumber && fetchIssueDetails(+issueNumber).then(setIssue)
  }, [issueNumber, fetchIssueDetails])
  ...
  return (
    <Container className={classes.content}>
      <Container className={classes.header}>
        ...
        <Title className={classes.title}>{issue.title}</Title>

        <Container className={classes.infos}>
          <Text>{issue.author}</Text>

          <Text>{formatTimePassed(issue.createdAt)}</Text>
      ...
      {hasComments && <Divider className={classes.divider} />}
